    ...
    if (mode == -1) {
	    ...
	    return;
            ...
            if (!(cm.getJob()==410 ||cm.getJob()==420)) {
				...
				cm.sendOk("#k#e你还没有到70级，不能进行飞侠第三次转职！");
				cm.dispose();
				return;		
            }	
			if (cm.haveItem(4031057, 1)){
				...
				cm.warp(103000003);
                cm.sendOk("#k#e请先去找一转教官#r达克鲁#k完成你在金银岛的第一阶段考验再来找我。");
                cm.dispose();
            ...
            if (cm.haveItem(4031058, 1)) {
                if (cm.getJob()==410) {
					...
					cm.sendOk("#e#r恭喜你成功三转为无影人！");
					cm.worldMessage("【转职信息】：恭喜玩家"+ cm.getChar().getName() +"进行三转成为无影人让我们热烈的祝福他/她吧！");
                    cm.dispose();
					...
					cm.gainItem(4031057, -1);
					cm.gainItem(4031058, -1);
                    cm.sendOk("#e#r恭喜你成功三转为独行客！");
					cm.worldMessage("【转职信息】：恭喜玩家"+ cm.getChar().getName() +"进行三转成为独行客让我们热烈的祝福他/她吧！");
                    cm.dispose();
                }
            } else if (cm.haveItem(4031057, 1))
